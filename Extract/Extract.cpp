...
Data extractData(string filename) {
    ...
    list<Room> rooms;
    list<Constraint> softConstraints, hardConstraints;
    ...
    for (i=0; i<nOfRooms; i++) {
        ...
        capacity = stod(v[2]);
        sizeLA = stoi(v[3]);
        for (j=4; j<v.size(); j++) adjacentRooms.push_back(stoi(v[j]));
    ...
    for (i=0; i<nOfHardConstraints+nOfSoftConstraints; i++) {
        ...
        if (constraintType == 1) hardConstraints.push_back(c);
        else softConstraints.push_back(c);
    }
    readFile.close();
    Data myData(
        nOfEntities,
